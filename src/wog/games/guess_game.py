...
A random number, 0 or higher will be selected by the game. you must guess that number.
...
def generate_number(max_number: int) -> int:
    "Generates a random number between 0 and max_number"
    return random.randint(0, max_number)


def get_guess_from_user(max_number: int) -> int:
    "Retrieves the guess from the user"
    while True:
        guess: str = input(f"please enter a number between 0 and {max_number}: ")
        parsed_guess: int | None = input_validation.validate_int_in_range(
            guess,
            0,
            max_number,
            input_not_number_prompt="Please enter a number",
            input_not_int_prompt="Please enter an integer",
            input_not_in_range_prompt="Input is not in the specified range",
        )
...
def compare_results(user_guess: int, number: int) -> bool:
    "Checkes if the user's guess and the actual number are the same"
